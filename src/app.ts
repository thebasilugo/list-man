...
function undo(): void {
  if (historyIndex > 0) {
    historyIndex--
    Object.assign(state, JSON.parse(JSON.stringify(historyStack[historyIndex])))
    renderTodos()
  }
}
...
function loadStateFromStorage(): void {
  ...
  state.theme = (localStorage.getItem(STORAGE_KEYS.THEME) || "light") as "light" | "dark"
}
...
function renderTodos(): void {
  if (elements.listContainer) {
    const filteredTodos = filterTodos(state.todos, state.filter).filter((todo) =>
    ...
    elements.listContainer.innerHTML = sortedTodos
      .map(
        (todo) => `
          <li class="list-item flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200 ${
            todo.completed ? "opacity-60" : ""
          } ${state.selectedTodos.has(todo.id) ? "bg-blue-100 dark:bg-blue-800" : ""}" data-id="${todo.id}">
            <div class="flex items-center space-x-3 flex-grow">
              <input type="checkbox" class="todo-checkbox w-5 h-5" ${todo.completed ? "checked" : ""}>
              <span class="todo-text flex-grow break-words ${todo.completed ? "line-through" : ""}">${todo.text}</span>
            </div>
...
function handleListClick(e: MouseEvent): void {
  ...
  if (target.closest(".delete-btn")) {
    ...
    updateTodoPriority(todoId, (target as HTMLSelectElement).value as "low" | "medium" | "high")
...
function toggleEdit(id: string): void {
  const listItem = elements.listContainer.querySelector(`[data-id="${id}"]`) as HTMLElement
  const todoText = listItem.querySelector(".todo-text") as HTMLElement
  const editInput = document.createElement("input")
  editInput.type = "text"
  editInput.className = "todo-edit-input bg-white dark:bg-gray-700 dark:text-white border rounded px-2 py-1 flex-grow"
...
function updateTodoPriority(id: string, priority: "low" | "medium" | "high"): void {
  const todo = state.todos.find((t) => t.id === id)
  if (todo) {
...
function initializeSortDropdown(): void {
  elements.sortDropdown?.addEventListener("click", (e: Event) => {
    ...
    if (target.classList.contains("item")) {
      state.sort = target.dataset.value as "created" | "alphabetical" | "priority"
      renderTodos()
    }
  })
}
...
$$(".filter-btn").forEach((button) => {
  button.addEventListener("click", (e: Event) => {
    const filter = (e.target as HTMLElement).id.replace("filter-", "") as "all" | "active" | "completed"
